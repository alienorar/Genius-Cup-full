...
export async function getResults(payload: IGetResultPayload): Promise<IResultsResponse> {
  try {
    ...
    throw new Error(error.response?.data?.message || "Ma'lumotlarni yuklashda xatolik yuz berdi");
  }
}
...
export async function getOlympiads(): Promise<IOlympiad[]> {
  try {
    ...
    console.error("Got error while fetching olympiads:", error);
    return [];
  }
}
...
export async function getTests(token: string): Promise<ITest[]> {
  if (!token) return [];
  try {
    const response: AxiosResponse<{ Tests: ITest[] }> = await axios.get(
      `${import.meta.env.BASE_URL}/test/admin/list`,
      {
        headers: {
    ...
    console.error("Got error while fetching tests:", error);
    return [];
  }
}
...
export async function getOtherCountryResults(payload: IOtherCountryPayload): Promise<IResultsResponse> {
  const { page, limit, testId, token } = payload;
  try {
    ...
    console.error("Error fetching other country results:", {
      message: error.message,
      response: error.response?.data,
      status: error.response?.status,
      payload: payload,
...
export async function getSingleResult(chatId: string | number): Promise<IResult | null> {
  try {
    ...
    console.error("Got error while fetching single result:", error);
    return null;
  }
}